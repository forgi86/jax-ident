...
_, params = model.init_with_output(subkey, jnp.ones((cfg.batch_size, cfg.nx)), u)
key, subkey = random.split(key)
_, params_est = estimator.init_with_output(
    subkey, jnp.ones((cfg.batch_size, cfg.seq_len_est * (cfg.nu + cfg.ny)))
)
...
for epoch in range(cfg.epochs):
    for idx, (batch_u, batch_y) in tqdm(enumerate(train_loader)):
        ...
        batch_est = jnp.concatenate(
            (
                batch_u_est.reshape(cfg.batch_size, -1),
                batch_y_est.reshape(cfg.batch_size, -1),
            ),
            axis=-1,
        )
        batch_x0 = estimator.apply(params_est, batch_est)
